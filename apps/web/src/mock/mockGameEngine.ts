...
export function createMockGameState(playerCount = MOCK_PLAYER_COUNT): GameState {
  ...
  const currentPlayerId = players[0]?.id ?? 'player-red'
...
function safeTrackIndices(state: GameState) {
  ...
  state.players.forEach((_, index) => {
    const layout = boardLayout.players[index]
    if (layout) {
...
function playerIndexById(state: GameState, playerId: string) {
  ...
  state.tokens
    ...
    .forEach((token) => {
      ...
      if (path.to.state === 'on_track') {
        ...
        const currentPlayerIndex = playerIndexById(state, token.playerId)
        const destinationAbsoluteTrack = absoluteTrackIndexFor(currentPlayerIndex, path.to.position)
        ...
        const capturedToken = state.tokens.find((candidate) => {
          ...
          const candidateIndex = playerIndexById(state, candidate.playerId)
          if (candidateIndex < 0) {
            return false
          }

          return absoluteTrackIndexFor(candidateIndex, candidate.position) === destinationAbsoluteTrack
        })
...
export function resolveMockTurn(state: GameState, moveId?: string) {
  ...
  const captureCandidate =
    pathInfo.to.state === 'on_track'
      ? state.tokens.find((candidate) => {
          ...
          const currentPlayerIndex = playerIndexById(state, moveToken.playerId)
          const destinationAbsoluteTrack = absoluteTrackIndexFor(currentPlayerIndex, pathInfo.to.position)
          const candidateIndex = playerIndexById(state, candidate.playerId)
          if (candidateIndex < 0) {
            return false
          }
