...
export function resolveTurn(state: GameState, moveId?: string) {
  ...
  const nextTokens = state.tokens.map((token) => {
    if (token.id !== chosenMove.tokenId) {
      return token
    }
    ...
    return applyPathToToken(token, path.path)
  })

  const capturedToken = chosenMove.capturedTokenId
    ? nextTokens.find((token) => token.id === chosenMove.capturedTokenId) ?? null
    : null

  const nextTokensWithCapture = capturedToken
    ? nextTokens.map((token) => {
        if (token.id !== capturedToken.id) {
          return token
        }
  ...
  const mover = nextTokensWithCapture.find((token) => token.id === chosenMove.tokenId)
  if (!mover) {
    return state
  }

  const moveType = chosenMove.moveType
  const moveDetails: MockMoveEventDetails = {
    tokenId: chosenMove.tokenId,
    playerId: mover.playerId,
    moveType,
    from: { state: state.tokens.find((token) => token.id === chosenMove.tokenId)?.state ?? mover.state, position: 0 },
    to: { state: mover.state, position: mover.position },
    path: [],
  }

  const timestampMove = now()
  ...
  let nextState: GameState = {
    ...state,
    version: state.version + 1,
    tokens: nextTokensWithCapture,
    events: nextEvents,
    updatedAt: timestampMove,
  }

  const finishData = appendFinishEvents(nextState, nextTokensWithCapture, timestampMove, mover.playerId)
  nextState = {
    ...nextState,
    events: finishData.events,
  }

  const finishOrder = finishData.finishOrder
  if (shouldEndGameByFinishCount(nextState.players.length, finishOrder.length)) {
    return buildFinishedState(nextState, finishOrder[0], finishData.events, timestampMove)
  }

  const keepCurrentPlayer = diceResult === 6
  const nextPlayerIndex = getNextPlayerIndex(nextState, keepCurrentPlayer)
  const nextPlayer = nextState.players[nextPlayerIndex] ?? nextState.players[0]
