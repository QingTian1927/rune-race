...
import { getRunePlayer, handHasRoom } from './state.js'
...
export function closePlacementPhase(state: GameState, timestamp: number): GameState {
  ...
  for (const [playerId, flags] of Object.entries(honestyByPlayer)) {
    ...
    if (flags.placedCount > 0) {
      ...
      if (newStreak >= RUNE_HONESTY_STREAK_FOR_REWARD) {
        const rewardType = drawRuneCardType()
  ...
  return {
    ...next,
    turn: { ...next.turn, phase: 'waiting_roll' },
    phase: 'waiting_roll',
    rune: { ...next.rune!, placement: null },
  }
}

export function canRollDuringPlacement(state: GameState, now: number): boolean {
  const placement = state.rune?.placement
  if (!placement) return true
  return now >= placement.minCloseAt
}
...
export function placeMarker(
  ...
  if (!state.players.some((p) => p.id === displayedIdentityId)) {
